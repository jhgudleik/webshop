...
@section('content')
    <div class="container">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                <h1 class="mb-1">Категории — дерево</h1>
                <p class="text-muted mb-0">Перетаскивайте категории для изменения порядка и перемещайте подкатегории.</p>
            </div>

            <div class="d-flex align-items-center gap-3">
                <div id="savingIndicator" style="display:none; align-items:center; gap:8px; font-size:14px; color:#6c757d;">
    ...
    async function moveCategory(categoryId, parentId, position) {
        ...
        try {
            const response = await fetch(moveUrl, {
                ...
                body: JSON.stringify({ category_id: categoryId, parent_id: parentId, position: position })
            });

            const data = await response.json();
            if (!response.ok) throw new Error(data.message || 'Ошибка');

            const el = document.querySelector(`.category-node[data-id="${categoryId}"]`);
            if (el) el.dataset.parentId = parentId ?? '';
        } catch (error) {
